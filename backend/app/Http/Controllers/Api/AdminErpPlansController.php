...
class AdminErpPlansController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        $planes = Service::where('slug', 'like', 'erp-%')
            ->orderBy('price')
            ->get(['id', 'name', 'slug', 'price_label', 'module_keys', 'costo_operacional', 'is_popular']);
    ...
    public function update(Request $request, Service $service): JsonResponse
    {
        ...
        $request->validate([
            ...
            'module_keys.*' => ['string'],
            'costo_operacional' => ['nullable', 'array'],
        ]);
        ...
        $costoOp = $request->costo_operacional ?? [];
        // Solo guardar costos de módulos activos
        $costoFiltrado = array_filter($costoOp, fn($k) => in_array($k, $moduleKeys, true), ARRAY_FILTER_USE_KEY);

        $service->update([
            'module_keys' => $moduleKeys,
            'costo_operacional' => $costoFiltrado,
        ]);
        ...
        return response()->json([
            'success' => true,
            'plan' => $service->fresh(['id', 'name', 'slug', 'price_label', 'module_keys']),
        ]);
    }
